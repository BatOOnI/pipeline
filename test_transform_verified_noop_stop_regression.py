import os
import shutil
import uuid

import agent_loop
import config


def _set_config(name, value, stash):
    stash[name] = getattr(config, name)
    setattr(config, name, value)


def main():
    root = os.path.join(os.getcwd(), "TEST", f"transform_verified_noop_{uuid.uuid4().hex[:8]}")
    os.makedirs(root, exist_ok=True)
    source = os.path.join(root, "source.txt")
    with open(source, "w", encoding="utf-8") as handle:
        handle.write("simple source body\n")

    logs = []
    backup = {}
    original_call_model = agent_loop.call_model
    call_count = {"value": 0}

    responses = [
        '{"actions":[{"type":"write_file","args":{"path":"calc_app.py","content":"a = 1\\nb = 2\\nprint(a + b)\\n"}}]}',
        '{"actions":[{"type":"write_file","args":{"path":"calc_app.py","content":"a = 1\\nb = 2\\nprint(a + b)\\n"}}]}',
        "done",
    ]

    def fake_call_model(*args, **kwargs):
        idx = call_count["value"]
        call_count["value"] += 1
        if idx < len(responses):
            return responses[idx]
        return "done"

    try:
        _set_config("PROJECT_ROOT", root, backup)
        _set_config("SESSION_FILE", ".agent/session_transform_verified_noop.json", backup)
        _set_config("PATCH_FILES", "", backup)
        _set_config("MODE_CONTROL", "AUTO", backup)
        _set_config("RESCUE_MODE", "OFF", backup)
        _set_config("MAX_ITERATIONS", 8, backup)
        _set_config("AUTO_VERIFY_PYTHON", True, backup)
        _set_config("AUTO_GIT_CHECKPOINTS", False, backup)
        _set_config("ALLOW_EMPTY_DONE_RETRY", False, backup)

        agent_loop.call_model = fake_call_model
        prompt = (
            "transform and copy source.txt to create calc_app.py, then verify with "
            r"C:\Users\B\AppData\Local\Programs\Python\Python314\python.exe -m py_compile calc_app.py"
        )
        agent_loop.run(prompt, logger=lambda msg: logs.append(str(msg)))

        assert os.path.exists(os.path.join(root, "calc_app.py")), "calc_app.py missing"
        assert any("TRANSFORM VERIFY OK" in line for line in logs), "transform verify should pass"
        assert any("VERIFY OK: calc_app.py" in line for line in logs), "python verify should pass"
        assert any(
            "TRANSFORM VERIFIED NO-OP -> STOP" in line or "TRANSFORM OUTPUTS VERIFIED -> STOP" in line
            for line in logs
        ), "run should terminate after verified no-op state"
        assert any("RUN SUMMARY:" in line and "status=success" in line for line in logs), "run should finish successfully"
        assert call_count["value"] == 1, f"unexpected extra iteration count: {call_count['value']}"
    finally:
        agent_loop.call_model = original_call_model
        for key, value in backup.items():
            setattr(config, key, value)
        shutil.rmtree(root, ignore_errors=True)

    print("OK: transform verified no-op stop regression checks passed")


if __name__ == "__main__":
    main()
